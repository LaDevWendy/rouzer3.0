...
<nav class="nav-categorias" class:visible class:oculta>
    <div class="colapsar-categorias categoria favorita" on:click={toggle}>
        <div class="fe fe-circle" />
        <Ripple color="var(--color5)" />
    </div>
    <a
        href="/Favoritas"
        title="favoritas"
        class="colapsar-categorias favorita categoria"
        style="display: flex;align-items: center;"
        ><span class="fe fe-star" /></a
    >
    <div
        title="Configurar favoritas"
        class="colapsar-categorias fe fe-settings categoria favorita cpt"
    ...
    {/each}
    <span
        class="categoria sep favorita"
        style="border-radius: 0 20px 20px 0;color:transparent; margin-right:4px"
        >.</span
    >
    <!-- 
    ...
    <span
        class="categoria sep"
        style="border-radius: 0 20px 20px 0;color:transparent">.</span
    >
</nav>
...
{#if configurando}
    <Dialog width="600" bind:visible>
            ...
            <div class="favoritas container-categorias cpt">
                {#each config.categorias.filter( (c) => favoritas.includes(c.id) ) as c (c.id)}
                    <span
                        ...
                        href="/{c.nombreCorto}"
                        on:click={() =>
                            ($localConfig.favoritas =
                                $localConfig.favoritas.filter(
                                    (c1) => c1 != c.id
                                ))}
                        title={c.nombre}>{c.nombreCorto}</span
                    >
                {/each}
            </div>
        ...
        <div slot="actions" class="actions center">
            <Button color="primary" on:click={() => (configurando = false)}
                >Ok</Button
            >
        </div>
    </Dialog>
...
<style>
    ...
    :not(.favorita) + .favorita .comunacha:last-of-type {
