...
<main>
    ...
    <section>
        ...
        {:else if !modoRegistro}
            <h2>
                Para crear y responder rozes en {config.nombre} debes iniciar una
                sesion
            </h2>
            <h3>
                No tenes cuenta? Enfermo!, <a
                    on:click={() => (modoRegistro = true)}
                    href="#Registro"
                    style="color:var(--color5) ">Registrate ahora mismo aca</a
                >
            </h3>
        {:else}
            <h2>
