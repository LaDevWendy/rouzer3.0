    ...
    Signal.coneccion.on("NuevaNotificacion", (noti) => {
        ...
        for (const n of notificaciones) {
            if (n.hiloId == noti.hiloId && n.tipo == noti.tipo && n.tipo == 0) {
                ...
                n.comentarioId == noti.comentarioId &&
                n.tipo == noti.tipo &&
                n.tipo == 1
            ) {
                n.conteo++;
...
<svelte:head>
    <title
        >{totalNotificaciones != 0 ? `(${totalNotificaciones})` : ""}
        {titulo}</title
    >
</svelte:head>
...
<span style="position: relative;">
    ...
    {#if mostrar}
        <ul
            ...
            class="notis panel drop-menu abs lista-nav menu1"
            on:mouseleave={() => (mostrar = false)}
        >
            {#each notificaciones as n}
                <a
                    href="/Notificacion/{n.id}?hiloId={n.hiloId}&comentarioId={n.comentarioId}"
                >
                    <li class="noti">
