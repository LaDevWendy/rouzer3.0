<script>
    ...
    if (protocoloMessi)
        version = `Protocolo Messi activado - Version Chad ${window.config.general.version}`;
    else if (protocoloSerenito)
        version = `Protocolo Serenito activado - Version Chad ${window.config.general.version}`;
</script>
...
>
        ...
        <div class="nav-principal" class:modoSticky={scrollY > 50}>
            ...
            {#if !protocoloMessi || $globalStore.usuario.esMod}
                <span
                    ...
                    on:click={() => (mostrarFormularioHilo = true)}
                >
                    <span
                        style="width:max-content; margin-right: 6px;cursor: pointer;"
                        >ROZ</span
                    >
                    <span class="fe fe-plus-square" />
                    <Ripple />
                </span>
            {/if}
    ...
    <NavCategorias visible={!ocultarCategorias} />
</header>
...
<style>
    ...
    .nav-principal > *,
    .nav-principal nav-botones span {
        /* height: 48px; */
        display: flex;
        align-items: center;
        /* margin-bottom: 10px; */
